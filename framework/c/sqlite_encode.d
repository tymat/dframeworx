/*
	This is a modified version of SQLite's encode.h for use with D.
	This code is public domain.
*/


module sqlite_encode;
...
debug(SQLITE_ENCODE_TEST)
{
	private import std.random;
	private import std.c.stdlib, std.c.stdio;
	
	
	// Avg size increase: 0.895%
	int main()
	{
	  ...
	  for(i=0; i<inp.length; i++){
		 ...
		 if( i%100==0 ){
			...
			for(j=0; j<n; j++) inp[j] = std.random.rand() & 0xff;
		 }
		 nByteIn += n;
		 nOut = sqlite_encode_binary(inp, n, outp);
		 nByteOut += nOut;
		 if( nOut!=strlen(outp) ){
			printf(" ERROR return value is %d instead of %d\n", nOut, strlen(outp));
			exit(1);
		 }
		 if( nOut!=sqlite_encode_binary(inp, n, null) ){
			printf(" ERROR actual output size disagrees with predicted size\n");
			exit(1);
		 }
		 m = (256*n + 1262)/253;
		 printf("size %d->%d (max %d)", n, strlen(outp)+1, m);
		 if( strlen(outp)+1>m ){
		 ...
		 j = sqlite_decode_binary(outp, outp);
		 if( j!=n ){
			printf(" ERROR decode size %d\n", j);
			exit(1);
		 }
		 if( memcmp(inp, outp, n)!=0 ){
	  ...
	  fprintf(stderr,"Avg size increase: %.3f%%\n",
		 (nByteOut-nByteIn)*100.0/cast(double)nByteIn);
